...
以下代码都十分“直观”，对照对应的原理进行理解即可。

### 7.1 pic_init

原理说明：

- [ICW1 连接 / 触发方式](#31-icw1-连接--触发方式)
- [ICW2 向量号](#32-icw2-向量号)
- [ICW3 级联方式](#33-icw3-级联方式)
- [ICW4 工作模式](#34-icw4-工作模式)
- [OCW1 屏蔽字](#35-ocw1-屏蔽字)
...
- [OCW2 结束方式](#36-ocw2-结束方式)

```c
void send_eoi(int vector) {
    // 如果中断来自主片，只需要向主片 OCW2 发送 EOI
    if (vector >= 0x20 && vector < 0x28) {
        outb(PIC_M_CTRL, PIC_EOI);
    }
    // 如果中断来自从片，除了向从片 OCW2 发送 EOI 以外，还要再向主片 OCW2 发送 EOI
