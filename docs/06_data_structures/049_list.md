...
在 `lib/list.c` 处实现测试函数（由于目前只有 `kalloc()` 和 `kfree()` 具备在虚拟内存空间中，分配和释放内存的能力，所以我们引入 `memory.h` 来使用）：
...
void list_test() {
    ...
    count = 3;
    while (count--) {
        node = (list_node_t *)kalloc(1);        // should be 0x105000, 0x106000, 0x107000
    ...
    node = (list_node_t *)kalloc(1);            // should be 0x105000
    list_push_back(list, node);
    ...
    list_remove(node);
    kfree((u32)node, 1);                        // should be 0x105000
}
```
